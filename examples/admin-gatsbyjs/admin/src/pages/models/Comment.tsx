...
import {
  ...
  useFindManyCommentCountQuery,
  useFindManyCommentQuery,
  useGetModelQuery,
} from '../../generated';
...
const Comment: React.FC<IPageProps> = ({ location }) => {
  const [pageSize, setPageSize] = useState(10);
  const [update, setUpdate] = useState<CommentFragmentFragment>();
  ...
  const { data, loading, fetchMore } = useFindManyCommentQuery({
    variables: {
      where,
      orderBy,
      first: pageSize,
  ...
  const { data: dataCount, loading: loadingCount } = useFindManyCommentCountQuery({
    variables: {
      where,
    },
  });
  ...
  const onAction = (action: 'create' | 'update' | 'delete', id?: number) => {
    ...
    } else if (action === 'update' && id && data?.findManyComment) {
      setUpdate(data.findManyComment.find((item) => item.id === id));
  ...
  return (
    <>
      <SEO title="Comment" keywords={['OAH', 'application', 'react']} />
      <Row>
        ...
        <Col breakPoint={{ xs: 12 }}>
          <Table
            ...
            data={data?.findManyComment || []}
            fetchMore={fetchMoreHandler}
            loading={loading || loadingCount}
            filterHandler={filterHandler}
            sortByHandler={sortByHandler}
            initialFilter={initialFilter}
            pageCount={dataCount?.findManyCommentCount ? Math.ceil(dataCount?.findManyCommentCount / pageSize) : 0}
          />
        </Col>
      </Row>
    </>
